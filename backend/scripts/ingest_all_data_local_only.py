"""
LOCAL RAG INGESTION SCRIPT (ChromaDB ONLY)

Ingests ALL data sources into LOCAL ChromaDB fallback ONLY.
Does NOT touch Qdrant Cloud.
...
⚠️ NOTE: This script is for LOCAL ChromaDB ONLY.
         For Qdrant Cloud ingestion, use: scripts/ingest_all_data.py
"""
...
def clear_vector_db():
    ...
    if VECTOR_DB_DIR.exists():
        logger.info(f"🗑️ Clearing LOCAL ChromaDB: {VECTOR_DB_DIR}")
...
async def main():
    logger.info("=" * 70)
    logger.info("🏠 LOCAL RAG INGESTION - ChromaDB ONLY (NOT QDRANT)")
    logger.info("=" * 70)
    logger.warning("⚠️  This script ONLY updates LOCAL ChromaDB fallback")
    logger.warning("⚠️  Qdrant Cloud will NOT be touched")
    logger.info("=" * 70)
    ...
    # ------------------------------------------------------------------
    logger.info("\n" + "=" * 70)
    logger.info("CLEAR OLD LOCAL DATA?")
    logger.info("=" * 70)
    logger.info("1. 🗑️  Clear EVERYTHING (local vector DB + managed content)")
    logger.info("2. 🗑️  Clear local vector DB only (fresh start, keep registry)")
    ...
    if choice == "1":
        clear_vector_db()
        clear_rag_content()
        logger.info("✅ Complete local reset - starting fresh")
    elif choice == "2":
        clear_vector_db()
        logger.info("✅ Local vector DB cleared - will rebuild from scratch")
    else:
        logger.info("➕ Will ADD to existing local data")

    # ------------------------------------------------------------------
    # Confirmation
    # ------------------------------------------------------------------
    logger.info("\n" + "=" * 70)
    logger.info(f"⚠️  READY TO INGEST {total_files} FILES INTO LOCAL CHROMADB")
    logger.info("=" * 70)
    logger.warning("⚠️  This will ONLY update the LOCAL fallback database")
    logger.warning("⚠️  Qdrant Cloud will remain UNCHANGED")
    logger.info("=" * 70)

    response = input("Continue local ingestion? (yes/no): ").strip().lower()
    ...
    # ------------------------------------------------------------------
    # Initialize RAG system (ChromaDB ONLY - NO QDRANT)
    # ------------------------------------------------------------------
    logger.info("\n🚀 Initializing LOCAL ChromaDB RAG system...")
    logger.info("🔒 Qdrant disabled for this ingestion")
    
    # Initialize with NO Qdrant credentials - ChromaDB ONLY
    vector_store = VectorStoreManager(
        persist_directory=str(VECTOR_DB_DIR),
        embedding_model_name="all-MiniLM-L6-v2",
        # Explicitly NO Qdrant - force ChromaDB only
        qdrant_host=None,
        qdrant_api_key=None,
        qdrant_dim=None
    )
    
    # Verify it's using ChromaDB only
    if vector_store.qdrant_client is not None:
        logger.error("❌ ERROR: Qdrant client detected! This script should only use ChromaDB.")
        logger.error("❌ Aborting to prevent accidental Qdrant ingestion.")
        return
    
    logger.info("✅ LOCAL ChromaDB-only system initialized")
    logger.info("✅ Qdrant verified as disabled\n")
    ...
    if 'json_content' in all_stats and JSON_DIR.exists():
        logger.info("=" * 70)
        logger.info("📥 INGESTING JSON FILES INTO LOCAL CHROMADB")
        logger.info("=" * 70)
    ...
    if 'spiritual' in all_stats and SPIRITUAL_DIR.exists():
        logger.info("\n" + "=" * 70)
        logger.info("📥 INGESTING PDF/TEXT FILES INTO LOCAL CHROMADB")
        logger.info("=" * 70)
    ...
    # ------------------------------------------------------------------
    logger.info("\n" + "=" * 70)
    logger.info("✅ LOCAL CHROMADB INGESTION COMPLETE")
    logger.info("=" * 70)

    stats = content_manager.get_content_statistics()

    logger.info(f"\n📊 FINAL LOCAL CHROMADB STATE")
    ...
    logger.info("\n✨ LOCAL ChromaDB fallback ready!")
    logger.info("💡 Qdrant Cloud remains unchanged")
    logger.info("=" * 70)
